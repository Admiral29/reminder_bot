...
async def send_reminder(chat_id, text):
    try:
        ...
        if not users:
            await app.send_message(chat_id, "Нет участников.")
            return
        mentions = []
        for user in users:
            if user.username:
...
async def cancel_remind(client, message):
    chat_id = str(message.chat.id)
    if chat_id not in jobs or not jobs[chat_id]:
        await message.reply("Нет напоминаний.")
        return
    args = message.text.split()
    if len(args) == 1:
        jobs[chat_id] = []
        save_jobs()
        await message.reply("❌ Все отменены.")
        return
    try:
        rem_id = int(args[1])
        for r in jobs[chat_id]:
            if r["id"] == rem_id:
                ...
                save_jobs()
                await message.reply(f"❌ Отменено ID {rem_id}.")
                return
        await message.reply(f"ID {rem_id} не найден.")
    except ValueError:
